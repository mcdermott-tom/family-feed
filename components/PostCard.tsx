...
export function PostCard({ post, userId }: { post: any, userId: string | null }) {
  ...
  return (
    <>
      ...
      <div className={`border-b border-zinc-100 dark:border-zinc-800 p-5 bg-white dark:bg-black text-black dark:text-white ${post.is_optimistic ? 'opacity-40' : ''}`}>
        ...
        <div className="mt-2">
          {post.is_optimistic ? (
            <div className="flex items-center gap-2 text-zinc-300 text-sm font-bold uppercase tracking-tighter">
              <Heart size={20} className="fill-zinc-100" /> Wait
            </div>
          ) : (
